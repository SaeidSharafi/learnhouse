...
async def check_enrollment_access(resource_uuid: str, user_id: int, db_session: AsyncSession) -> bool:
    ...
    if group_ids:
        grouped_offers = (await db_session.exec(
...
async def check_activity_paid_access(
    ...
    activity = (await db_session.exec(select(Activity).where(Activity.id == activity_id))).first()
    if not activity:
        raise HTTPException(status_code=404, detail="Activity not found")

    course = (await db_session.exec(select(Course).where(Course.id == activity.course_id))).first()
    if not course:
    ...
    direct_resources = (await db_session.exec(
        select(PaymentsOfferResource).where(
...
async def check_course_paid_access(
    ...
    """
    course = (await db_session.exec(select(Course).where(Course.id == course_id))).first()
    if not course:
        raise HTTPException(status_code=404, detail="Course not found")
