...
async def create_payments_offer(
    ...
    config = (await db_session.exec(select(PaymentsConfig).where(PaymentsConfig.org_id == org_id))).first()
    ...
    await db_session.refresh(new_offer)
...
async def delete_payments_offer(
    ...
    active_enrollments = (await db_session.exec(
