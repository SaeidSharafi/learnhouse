...
async def api_list_public_offers(
    ...
    async def _enrich_resources(offer_id: int, group_id) -> list[dict]:
        uuids: list[str] = []
        direct = (await db_session.exec(
            select(PaymentsOfferResource).where(PaymentsOfferResource.offer_id == offer_id)
        )).all()
        uuids.extend(r.resource_uuid for r in direct)
        if group_id:
            group_res = (await db_session.exec(
    ...
    result = []
    for o in offers:
        resources = await _enrich_resources(o.id, o.payments_group_id)
        result.append({
            "id": o.id,
...
async def api_get_offers_by_resource(
    ...
    direct_rows = (await db_session.exec(
    ...
    group_resource_rows = (await db_session.exec(
    ...
    if group_ids:
        group_offers = (await db_session.exec(
            select(PaymentsOffer).where(
...
async def api_get_public_offer(
    ...
    if offer.payments_group_id:
        group_resources = (await db_session.exec(
            select(PaymentsGroupResource).where(
    ...
    for uuid in uuids:
        if uuid.startswith("course_"):
            course = (await db_session.exec(select(Course).where(Course.course_uuid == uuid))).first()
            if course:
                enriched.append({
                    "resource_uuid": uuid,
                    "resource_type": "course",
