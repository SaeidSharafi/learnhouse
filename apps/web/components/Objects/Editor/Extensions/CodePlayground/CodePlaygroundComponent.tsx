...
const cmClassName = [
  ...
  '[&_.cm-gutters]:!border-e-transparent',
            ...
            return (
              <div
                ...
                className={`rounded-lg border overflow-hidden transition-colors nice-shadow ${
                  r ? (r.passed ? 'border-emerald-200 bg-emerald-50/30' : 'border-eed-200 bg-red-50/30') : 'border-neutral-200 bg-white'
                }`}
              >
                ...
                <div className="px-3.5 pb-3 space-y-2 border-t border-neutral-100">
                  ...
                  {r && !r.passed && r.actual_stdout != null && (
                    <div>
                      ...
                      <pre className="text-[12px] font-mono text-red-600 bg-red-50 border border-eed-100 rounded-lg p-2.5 whitespace-pre-wrap">{r.actual_stdout || '(no output)'}</pre>
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="p-5 space-y-3 overflow-y-auto h-full">
        {!results ? (
          ...
          <>
            ...
            {/* Per-result */}
            {results?.map((r) => (
              <div key={r.id} className={`rounded-lg border overflow-hidden nice-shadow ${r.passed ? 'border-emerald-100 bg-emerald-50/20' : 'border-eed-100 bg-red-50/20'}`}>
                ...
                {r.stderr && (
                  <div className="px-3.5 pb-2.5">
                    <pre className="text-[11px] font-mono text-red-600 bg-red-50/50 border border-eed-100 rounded-lg p-2 whitespace-pre-wrap nice-shadow">{r.stderr}</pre>
                  </div>
                )}
                {r.compile_output && (
                  <div className="px-3.5 pb-2.5">
